...
public final class ImageMapper {
    public Texture map(Image image) {
        ...
        int width = minMip < 0 ? image.header().pixelWidth() : image.mips().get(minMip).mipPixelWidth();
        int height = minMip < 0 ? image.header().pixelHeight() : image.mips().get(minMip).mipPixelHeight();
    ...
    private List<Surface> convertMipMaps(Image image) {
        ...
        int mipCount = image.mips().size() / faces;
        ...
        for (int face = 0; face < faces; face++) {
            for (int mip = minMip; mip < mipCount; mip++) {
                ...
                surfaces.add(new Surface(
                    image.mips().get(mipIndex).mipPixelWidth(),
                    image.mips().get(mipIndex).mipPixelHeight(),
