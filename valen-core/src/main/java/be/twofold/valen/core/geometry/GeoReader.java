...
public interface GeoReader<T> {
    ...
    private static Vector3 readVector3UNorm8Normal(BinarySource source) throws IOException {
        float x = Math.fma(FloatMath.unpackUNorm8(source.readByte()), 2.0f, -1.0f);
        float y = Math.fma(FloatMath.unpackUNorm8(source.readByte()), 2.0f, -1.0f);
        float z = Math.fma(FloatMath.unpackUNorm8(source.readByte()), 2.0f, -1.0f);
