...
public record StaticModel(
    ...
    public static StaticModel read(DataSource source, int version) throws IOException {
        ...
        var meshInfos = source.readStructs(header.numSurfaces(), source1 -> StaticModelMeshInfo.read(source1, version));
        if (version < 81) {
            source.readInts(3);
        }
        var textureAxis = source.readStructs(source.readInt(), s -> StaticModelTextureAxis.read(s, version));
        var geoDecals = StaticModelGeoDecals.read(source);
        var streamedLods = source.readStructs(header.numSurfaces() * header.numLods(), DataSource::readBoolByte);
